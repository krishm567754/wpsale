...
function cleanDate(val) {
  if (!val) return 'N/A';
  var dt;
  if (typeof val === 'number') { dt = new Date(Math.round((val - 25569) * 86400000)); } else { dt = new Date(val); }
...
// ✅ FIXED: Parse pipe-separated PDF tables (not comma!)
...
// ✅ FIXED: Normalize size headers to match user queries (0.9L = 900ml, 1l = 1L, etc.)
...
// ✅ FIXED: Format product chunk with normalized sizes for AI
function formatProductChunk(productName, headers, values, type) {
  ...
  for (var i = 1; i < headers.length && i < values.length; i++) {
    ...
    var value = values[i];
    if (header && value && !isNaN(parseFloat(value)) && header.toLowerCase() !== 'brand name') {
      var normalizedSize = normalizeSizeHeader(header);
      lines.push('- Size [' + normalizedSize + '] : Rs. ' + value);
    }
  }
  return lines.join('\n');
}

// ✅ FIXED: Tokenized fuzzy search with proper PDF parsing
function searchProducts(query, mrpTextRaw, dlpTextRaw) {
  ...
  function scanText(text, type) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      // Detect header row (contains size units or "brand name")
      ...
      if (name.length < 4 || lower.indexOf('passenger car') !== -1 || lower.indexOf('diesel engine') !== -1 || lower.indexOf('gear oils') !== -1 || lower.indexOf('greases') !== -1 || lower.indexOf('coolants') !== -1 || lower.indexOf('auto care') !== -1 || lower.indexOf('heavy duty') !== -1) continue;
...
module.exports = async function(req, res) {
  ...
  try {
    ...
    // ✅ PENDING REPLY (Number Selection) - Absolute Priority
    if (/^\d+$/.test(text)) {
      var pending = null;
    ...
    // ✅ Product Search with Fuzzy Logic + Fixed PDF Parsing
    ...
    // ✅ Invoice Search with Total Volume
