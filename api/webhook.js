...
// ─── ROBUST DATE RANGE EXTRACTOR (Supports custom ranges like "1 apr se 5 april") ─────────
function extractDateRange(text) {
    ...
    // Custom date range: "1 apr se 5 april" or "1/4/26 to 5/4/26"
    var customRange = text.match(/(\d{1,2})\s*(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|january|february|march|april|may|june|july|august|september|october|november|december)?\s*(\/\d{2})?\s*(se|to|tak|se)\s*(\d{1,2})\s*(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|january|february|march|april|may|june|july|august|september|october|november|december)?\s*(\/\d{2})?/i);
    if (customRange) {
        var d1 = parseInt(customRange[1]);
        var m1 = customRange[2] ? monthMap[customRange[2].toLowerCase().substring(0,3)] - 1 : cm;
        var y1 = customRange[3] ? (parseInt(customRange[3].replace('/','')) > 50 ? 1900 : 2000) + parseInt(customRange[3].replace('/','')) : cy;
        var d2 = parseInt(customRange[5]);
        var m2 = customRange[6] ? monthMap[customRange[6].toLowerCase().substring(0,3)] - 1 : cm;
        var y2 = customRange[7] ? (parseInt(customRange[7].replace('/','')) > 50 ? 1900 : 2000) + parseInt(customRange[7].replace('/','')) : cy;
        return { from: toSerial(y1, m1, d1), to: toSerial(y2, m2, d2), label: d1 + '/' + (m1+1) + ' to ' + d2 + '/' + (m2+1) };
    }
    ...
    for (var m in monthMap) {
        if (lower.indexOf(m) !== -1) {
            ...
            return { from: toSerial(y, mIdx, 1), to: toSerial(y, mIdx + 1, 0), label: m + ' ' + y };
        }
    }
    
    if (lower.match(/\btoday\b|\baaj\b/)) return { from: toSerial(cy, cm, cd), to: toSerial(cy, cm, cd), label: 'Today' };
    if (lower.match(/\byesterday\b|\bkal\b/)) return { from: toSerial(cy, cm, cd - 1), to: toSerial(cy, cm, cd - 1), label: 'Yesterday' };
    
    if (lower.match(/\bthis\s*week\b|\bis\s*hafte\b|\bchalu\s*hafte\b/)) {
        var d = now.getDay() === 0 ? 6 : now.getDay() - 1;
        return { from: toSerial(cy, cm, cd - d), to: toSerial(cy, cm, cd + (6 - d)), label: 'This Week' };
    }
    if (lower.match(/\blast\s*week\b|\bpichla\s*hafte\b|\bpichhle\s*hafte\b|\bprevious\s*week\b/)) {
        var d2 = now.getDay() === 0 ? 6 : now.getDay() - 1;
        return { from: toSerial(cy, cm, cd - d2 - 7), to: toSerial(cy, cm, cd - d2 - 1), label: 'Last Week' };
    }
    if (lower.match(/\bthis\s*month\b|\bis\s*month\b|\bchalu\s*mahine\b|\bis\s*mahine\b/)) {
        return { from: toSerial(cy, cm, 1), to: toSerial(cy, cm + 1, 0), label: 'This Month' };
    }
    if (lower.match(/\blast\s*month\b|\bpichla\s*mahine\b|\bpichhle\s*mahine\b|\bprevious\s*month\b|\bgaya\s*mahine\b/)) {
        return { from: toSerial(cy, cm - 1, 1), to: toSerial(cy, cm, 0), label: 'Last Month' };
    }
    return null;
}
...
function formatResponse(query, agg, rows) {
    ...
    if (query.type === 'customer_detail' && query.filters.customer) {
        ...
        if (query.filters.dateRange && query.filters.dateRange.label) lines.push('*Period: ' + query.filters.dateRange.label + '*');
        lines.push('');
        ...
        lines.push('*Sales Executive Report*'); if (query.filters.dateRange && query.filters.dateRange.label) lines.push('*Period: ' + query.filters.dateRange.label + '*'); lines.push('');
