...
async function loadExcelData() {
    ...
    // Limit data to stay within free model context limits
    return combined.slice(-15000) || 'No data.';
}
...
async function getAIReply(userMessage, excelData, systemPrompt) {
    ...
    try {
        console.log('[AI] Calling GPT-OSS-120B Free...');
        const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
            ...
            body: JSON.stringify({
                // Updated to GPT-OSS-120B Free ID
                model:      'openai/gpt-oss-120b:free',
                max_tokens: 800,
                temperature: 0.2, // Balance for accuracy and natural chat
                messages: [
                    {
                        role:    'system',
                        content: `${systemPrompt}\n\nKNOWLEDGE BASE (Excel Data):\n${excelData}`
                    },
        ...
        return d?.choices?.[0]?.message?.content?.trim() || 'No response from model.';

    } catch (e) {
        return 'Technical issue with AI. Thodi der mein try karein.';
    }
}
