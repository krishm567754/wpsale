...
// ✅ NEW: Robust PDF Price Parser (Converts messy tables to clean AI context)
function parsePriceList(rawText, type) {
  if (!rawText) return '';
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (!line || line.indexOf('As recommended') !== -1 || line.indexOf('---') !== -1) continue;

    var cells = line.split('|').map(function(c) { return c.trim(); });
    
    // Detect header row (contains size units)
    var isHeader = cells.some(function(c) { return c.match(/^\d+\.?\d*\s*(l|ml|kg|gm|pack)$/i); });
    if (isHeader) {
      currentHeaders = cells.map(function(c) {
        var m = c.match(/(\d+\.?\d*)\s*(l|ml|kg|gm|pack)/i);
    ...
    if (!cells[0] || (cells[0].length < 3 && cells.length < 5)) continue;
    if (cells[0].indexOf(type === 'MRP' ? 'DIESEL ENGINE OILS' : 'GEAR OILS') !== -1 || cells[0].indexOf('MOTORCYCLE OIL') !== -1 || cells[0].indexOf('PASSENGER CAR') !== -1) continue;

    // Parse data row
    var productName = cells[0];
    for (var j = 1; j < cells.length && j <= currentHeaders.length; j++) {
      if (currentHeaders[j-1] && cells[j] && !isNaN(parseFloat(cells[j]))) {
        results.push(productName + ' | SIZE:' + currentHeaders[j-1] + ' | ' + type + ': Rs.' + cells[j]);
      }
    }
  }
  return results.join('\n');
...
function searchInvoices(query, invoiceMap) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    var matchInv = invNo.replace(/[^a-zA-Z0-9]/g, '').toLowerCase().indexOf(q.replace(/[^a-zA-Z0-9]/g, '')) !== -1;
    var keywordScore = 0;
...
async function loadAllData() {
  ...
  var mrpText = mrpPdfUrl ? parsePriceList(await extractPDFText(mrpPdfUrl), 'MRP') : '';
  var listText = listPdfUrl ? parsePriceList(await extractPDFText(listPdfUrl), 'DLP') : '';
...
module.exports = async function(req, res) {
  ...
  try {
    ...
    // ✅ PENDING REPLY (Absolute Priority)
    ...
    // ✅ RATE QUERY (Clean Data Guaranteed)
