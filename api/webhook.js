...
function searchInvoices(query, invoiceMap) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    var matchCust = userKeywords.every(function(word) { return custName.indexOf(word) !== -1; });
    if (matchInv || matchCust) { matches.push({ invNo: invNo, rows: rows, customer: rows[0]['Customer Name'] }); }
  }
...
async function loadAllData() {
  ...
  var excelData = [lines.join('\n'), mrpText ? '\n\n[MRP DATA SECTION - ONLY USE FOR MRP]\n' + mrpText : '', listText ? '\n\n[DLP DATA SECTION - ONLY USE FOR DLP/LIST PRICE]\n' + listText : ''].join('');
...
module.exports = async function(req, res) {
  ...
  try {
    ...
    // ✅ GREETING CHECK (Restored & Priority)
