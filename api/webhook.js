...
async function getSystemPrompt() {
  var d = getFirebase();
  var def = 'Tu Krish hai - Shri Laxmi Auto Store, Bikaner ki WhatsApp Assistant.\n\nSTRICT RULES:\n1. Sirf provided data se exact rate batao. 0.9L = 900ml (ye dono same hain).\n2. MRP vs DLP: "MRP" ke liye SIRF [MRP DATA] section dekhein. "DLP/List Price" ke liye SIRF [DLP DATA] section dekhein.\n3. Exact Size ki value batayein jo user ne manga hai.\n4. Format: *Product:* Name (Size)\n*MRP:* Rs.X\n*DLP:* Rs.Y\n5. Text Hinglish me rakho aur directly jawab do. Agar size na mile to "Size available nahi hai" bolo.';
  if (!d) return def;
...
// ✅ PRINCIPLE 1: Smart CSV Parser for PDF Tables
...
// ✅ PRINCIPLE 1: Convert messy PDF rows into clean key-value chunks
function formatProductChunk(productName, headers, values, type) {
  var lines = ['Product: ' + productName];
  for (var i = 1; i < headers.length && i < values.length; i++) {
    var header = headers[i];
    var value = values[i];
    if (header && value && !isNaN(parseFloat(value))) {
      // Normalize size headers: "11" -> "1L", "51" -> "5L", etc.
      header = header.replace(/^11$/, '1L').replace(/^51$/, '5L').replace(/^71$/, '7L')
                     .replace(/^31$/, '3L').replace(/^2\.51$/, '2.5L').replace(/1\.2\/\s*11/, '1.2L / 1L')
                     .replace(/\/$/, 'L').replace(/\s+/g, ' ');
      lines.push('- Size [' + header + '] : Rs. ' + value);
    }
  }
  return lines.join('\n');
}

// ✅ PRINCIPLE 2: Tokenized Fuzzy Search for Products
function searchProducts(query, mrpTextRaw, dlpTextRaw) {
  ...
  function scanText(text, type) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (lower.indexOf('brand name') !== -1 || lower.match(/^\s*\|?\s*\d+\.?\d*\s*(l|ml|kg|gm|pack)/i)) {
      ...
      if (name.length < 4 || lower.indexOf('passenger car') !== -1 || lower.indexOf('diesel engine') !== -1 || lower.indexOf('gear oils') !== -1) continue;
      
      // ✅ Fuzzy matching: normalize and check token overlap
      var nameNorm = name.toLowerCase().replace(/[^a-z0-9]/g, ' ');
      var exactNameMatches = 0;
      for (var t = 0; t < searchTerms.length; t++) {
        if (nameNorm.indexOf(searchTerms[t]) !== -1) exactNameMatches++;
      }
      var requiredMatches = Math.min(2, Math.max(1, searchTerms.length - 1));
      
      if (exactNameMatches >= requiredMatches) {
        var chunk = formatProductChunk(name, currentHeaders, cols, type);
...
// ✅ Invoice Search with keyword scoring
...
async function loadAllData() {
  ...
  var mrpTextRaw = mrpPdfUrl ? await extractPDFText(mrpPdfUrl) : '';
  var listTextRaw = listPdfUrl ? await extractPDFText(listPdfUrl) : '';
...
module.exports = async function(req, res) {
  ...
  try {
    ...
    // ✅ PRINCIPLE 3: Pending Reply with Original Query Recall
    if (/^\d+$/.test(text)) {
      var pending = null;
      ...
      if (pending && pending.matches) {
        ...
        if (pending.matches[idx]) {
          if (pending.type === 'invoice') {
            ...
            // ✅ PRINCIPLE 3 + 4: Inject original query + explicit size rule
            var aiPrompt = 'User\'s ORIGINAL query was: "' + pending.originalQuery + '". Now User selected Product: ' + p.name + '. Give exact MRP and DLP for the SPECIFIC SIZE they originally asked for. Note: 0.9L is exactly equal to 900ml. If size not found, say "Size available nahi hai".';
    ...
    if (isRateQuery || (prodMatches.length > 0 && invMatches.length === 0)) {
      if (prodMatches.length === 1) {
        ...
        // ✅ PRINCIPLE 3: Save originalQuery with pending state
