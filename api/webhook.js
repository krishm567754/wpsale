...
// 🔐 Sanitize WhatsApp ID for Firebase paths (fixes @ . # $ [ ] / errors)
...
async function getSystemPrompt() {
  ...
  var def = 'Tu Laxmi hai - Shri Laxmi Auto Store, Bikaner ki WhatsApp Assistant.\n\n' +
    ...
    '2. MRP vs DLP SEPARATION: Agar user ne "MRP" bola to SIRF MRP column se rate batao. Agar "DLP" ya "List Price" bola to SIRF DLP/List Price column se rate batao. Dono mix mat karo.\n' +
    '3. SIZE MATCHING: Product name ke saath size (900ml, 1L, 4.5L, 9L, etc.) exact match karo. "GTX 900ml" ka rate alag hai, "GTX 4.5L" ka alag.\n' +
...
async function loadAllData() {
  ...
  if (mrpPdfUrl) { var raw = await extractPDFText(mrpPdfUrl); mrpText = raw.slice(0, 10000); }
  if (listPdfUrl) { var raw2 = await extractPDFText(listPdfUrl); listText = raw2.slice(0, 10000); }
  var excelData = [lines.join('\n'), mrpText ? '\n\nMRP PRICE DATA (Column: MRP):\n' + mrpText : '', listText ? '\n\nLIST PRICE/DLP DATA (Column: List Price/DLP):\n' + listText : ''].join('');
...
function detectIntent(text, mrpPdfUrl, listPdfUrl, mrpFile, listFile, savedPDFs) {
  ...
  for (var k in savedPDFs) {
    if (lower.indexOf(k.toLowerCase()) !== -1 && hasSend) return { type: 'pdf', pdf: savedPDFs[k] };
  }
...
module.exports = async function(req, res) {
  ...
  try {
    ...
    // 🔐 Sanitize from for Firebase paths
    ...
    // Handle numbered selection from previous list (with sanitized path)
    ...
    if (matches.length === 1) {
      ...
      if (database) {
        try { await database.ref('pending/' + safeFrom).set({matches: matches, ts: Date.now()}); } catch(e){ console.error('[SAVE] Err:', e.message); }
      }
      ...
      // User tried invoice search but no match found
      await sendText(from, 'Invoice nahi mila. Number ya customer name check karke dobara try karein.');
      return res.status(200).json({status:'ok'});
    }

    // 🤖 Fallback to AI for rates/general queries
    var aiReply = await getAIReply(text, dataResult.excelData, sysPrompt);
    if (!aiReply || aiReply.indexOf('Error')!==-1 || aiReply.indexOf('missing')!==-1) await sendText(from, 'Thoda wait karein ya product/invoice number clear likhein.');
