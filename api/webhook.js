...
function normalizeSizes(text) {
  if (!text) return '';
  return text.replace(/0\.9\s*l/gi,'900ML').replace(/0\.9l/gi,'900ML').replace(/900\s*ml/gi,'900ML').replace(/800\s*ml/gi,'800ML').replace(/600\s*ml/gi,'600ML').replace(/500\s*ml/gi,'500ML').replace(/1\s*l\b/gi,'1L').replace(/4\s*\.?\s*5l\b/gi,'4.5L').replace(/9\s*l\b/gi,'9L');
}
...
function findRateDirectly(query, mrpText, listText) {
  ...
  targetSize = targetSize.replace(/0\.9L/, '900ML');
  var namePart = q.replace(/(\d+\.?\d*ml|\d+\.?\d*l|0\.9l)/gi, '').replace(/\s+/g, ' ').trim();
  if (!namePart) return null;

  function extractPrice(textBlock) {
    if (!textBlock) return null;
    var lines = textBlock.split('\n');
    for (var i = 0; i < lines.length; i++) {
      var line = lines[i].toUpperCase().trim();
      if (line.indexOf(namePart.toUpperCase()) !== -1 && (line.indexOf(targetSize) !== -1 || targetSize === '')) {
        var prices = line.match(/(\d{2,5})/g);
        if (prices && prices.length > 0) return parseInt(prices[prices.length > 2 ? 2 : 0]);
      }
    }
    return null;
  }

  var mrp = extractPrice(mrpText);
  var dlp = extractPrice(listText);
  ...
  try {
    ...
    if (intent.type === 'ai_rate') {
      ...
      if (directRate) {
        ...
        if (!aiReply || aiReply.indexOf('Error') !== -1 || aiReply.indexOf('missing') !== -1) { await sendText(from, 'Thoda wait karein ya product naam aur size clear likhein (e.g., Activ 4T 20W-40 900ml).'); } else { await sendText(from, aiReply); }
      }
      return res.status(200).json({ status: 'ok' });
    }
    ...
    var aiReply = await getAIReply(text, dataResult.excelData, sysPrompt);
    if (!aiReply || aiReply.indexOf('Error') !== -1 || aiReply.indexOf('missing') !== -1) { await sendText(from, 'Thoda wait karein ya clear likhein.'); } else { await sendText(from, aiReply); }
    return res.status(200).json({ status: 'ok' });
  } catch (e) {
