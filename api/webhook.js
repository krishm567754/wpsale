...
function cleanPDFText(text) {
  if (!text) return '';
  // Remove repetitive headers and noise to keep AI focused
  return text
    .replace(/BRAND NAME|DIESEL ENGINE OILS|GEAR OILS\/ TRANSMISSION FLUIDS|GREASES|HEAVY DUTY|COOLANTS|AUTO CARE PRODUCTS|MOTORCYCLE OIL|PASSENGER CAR MOTOR OILS/gi, '')
    .replace(/As recommended by Castrol.*?\d{2}-\d{2}-\d{4}/gi, '')
    .replace(/\|\s*\|/g, ' | ')
    .replace(/\s+/g, ' ')
    .trim();
}
...
function searchInvoices(query, invoiceMap) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    var allMatch = userKeywords.every(function(word) { return custName.indexOf(word) !== -1; });
    if (allMatch) { matches.push({ invNo: invNo, rows: rows, customer: rows[0]['Customer Name'] }); }
  }
...
async function loadAllData() {
  ...
  var rawMrp = mrpPdfUrl ? await extractPDFText(mrpPdfUrl) : '';
  var rawList = listPdfUrl ? await extractPDFText(listPdfUrl) : '';
  var mrpText = normalizeSizes(cleanPDFText(rawMrp)).slice(0, 15000);
  var listText = normalizeSizes(cleanPDFText(rawList)).slice(0, 15000);
...
function detectIntent(text, mrpPdfUrl, listPdfUrl, mrpFile, listFile, savedPDFs) {
  ...
  var isRateQuery = ['rate','kya hai','kitna','price','mrp','dlp','kitne ka','dam','rupay'].some(function(w){return lower.indexOf(w)!==-1;});
...
module.exports = async function(req, res) {
  ...
  try {
    ...
    // ✅ GREETING CHECK (MOVED TO TOP)
    var lower = text.toLowerCase();
    if(['hi','hello','namaste','hey','hii','good morning'].some(function(g){return lower.indexOf(g)!==-1;})) {
      ...
      try {
        ...
        if(snap.exists()) {
          ...
          if(pending.matches && pending.matches[idx]) {
            var m=pending.matches[idx]; var f=m.rows[0]; var prods=m.rows.map(function(r){return r['Product Name']+'('+r['Product Volume']+'L)';}).join(' + ');
            var tG=m.rows.reduce(function(s,r){return s+(parseFloat(r['Total Value incl VAT/GST'])||0);},0); var wG=m.rows.reduce(function(s,r){return s+(parseFloat(r['Total Value Without GST'])||0);},0);
            var cg=m.rows.reduce(function(s,r){return s+(parseFloat(r['CGST Value'])||0);},0); var sg=m.rows.reduce(function(s,r){return s+(parseFloat(r['SGST Value'])||0);},0); var vl=m.rows.reduce(function(s,r){return s+(parseFloat(r['Product Volume'])||0);},0);
            ...
            await database.ref('pending/'+safeFrom).remove(); return res.status(200).json({status:'ok'});
          }
        }
      } catch(e){}
    }
    ...
    // ✅ RATE QUERY BYPASSES INVOICE SEARCH
    ...
    if(matches.length === 1) {
      var m=matches[0]; var f=m.rows[0]; var prods=m.rows.map(function(r){return r['Product Name']+'('+r['Product Volume']+'L)';}).join(' + ');
      var tG=m.rows.reduce(function(s,r){return s+(parseFloat(r['Total Value incl VAT/GST'])||0);},0); var wG=m.rows.reduce(function(s,r){return s+(parseFloat(r['Total Value Without GST'])||0);},0);
      var cg=m.rows.reduce(function(s,r){return s+(parseFloat(r['CGST Value'])||0);},0); var sg=m.rows.reduce(function(s,r){return s+(parseFloat(r['SGST Value'])||0);},0); var vl=m.rows.reduce(function(s,r){return s+(parseFloat(r['Product Volume'])||0);},0);
      await sendText(from,'*Invoice:* '+m.invNo+'\n*Customer:* '+f['Customer Name']+'\n*Products:* '+prods+'\n*Total:* Rs.'+tG.toFixed(2)+'\n*Tax:* CGST Rs.'+cg.toFixed(2)+' + SGST Rs.'+sg.toFixed(2)+'\n*Date:* '+cleanDate(f['Invoice Date'])+'\n*Payment:* '+f['Mode Of Payement']);
      return res.status(200).json({status:'ok'});
    } else if(matches.length > 1) {
      var msg='*Multiple matches found. Reply with number (1, 2, 3):\n\n';
    ...
    } else if(matches.length === 0 && (text.match(/^\d+$/) || text.toLowerCase().indexOf('inv') !== -1)) {
