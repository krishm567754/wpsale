...
async function getSystemPrompt() {
  ...
  var def = 'Tu Laxmi hai - Shri Laxmi Auto Store, Bikaner ki WhatsApp Assistant.\n\n' +
    ...
    '2. MRP vs DLP SEPARATION: Agar "MRP" bola to SIRF [MRP DATA] se batao. "DLP/List Price" bola to SIRF [DLP DATA] se batao.\n' +
    '3. SIZE MATCHING: 0.9L = 900ML, 1L = 1L, 4.5L = 4.5L. Exact size ka rate batao.\n' +
    '4. Format: *Product:* Name (Size)\n*MRP:* Rs.X\n*DLP:* Rs.Y (sirf agar DLP pucha ho)\n' +
    '5. Amounts ke saath Rs. lagao. Hinglish, max 5 lines. Sirf * use karo bold ke liye.';
  if (!d) return def;
...
// ✅ Normalize sizes & query terms
function normalizeSizes(text) {
  if (!text) return '';
  return text.replace(/0\.9\s*l/gi,'900ML').replace(/0\.9l/gi,'900ML')
             .replace(/900\s*ml/gi,'900ML').replace(/800\s*ml/gi,'800ML').replace(/600\s*ml/gi,'600ML')
             .replace(/500\s*ml/gi,'500ML').replace(/1\s*l\b/gi,'1L').replace(/4\s*\.?\s*5l\b/gi,'4.5L').replace(/9\s*l\b/gi,'9L');
}

// ✅ Smart Invoice Search (Returns similar matches if exact fails)
function searchInvoices(query, invoiceMap) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (matchInv || keywordScore > 0) {
      matches.push({ invNo: invNo, rows: rows, customer: rows[0]['Customer Name'], score: matchInv ? 10 : keywordScore });
    }
  }
  // Sort by score (exact invoice > keyword matches)
  matches.sort(function(a,b){ return b.score - a.score; });
  return matches.slice(0, 5);
}

// ✅ Direct Rate Lookup (Bypasses AI hallucination)
function findRateDirectly(query, mrpText, listText) {
  if (!mrpText || !listText) return null;
  var q = query.toLowerCase().replace(/rate|batao|bhi|hai|kya|mrp|dlp|list|price|of|ka|inr|rs|aur|and/gi, ' ').trim();
  var sizeMatch = q.match(/(\d+\.?\d*ml|\d+\.?\d*l|0\.9l)/i);
  var targetSize = sizeMatch ? sizeMatch[1].replace(/\s/g,'').toUpperCase() : '';
  targetSize = targetSize.replace(/0\.9L/, '900ML').replace(/1L/, '1L');
  
  var namePart = q.replace(/(\d+\.?\d*ml|\d+\.?\d*l|0\.9l)/gi, '').replace(/\s+/g, ' ').trim();
  if (!namePart) return null;

  function extractPrice(textBlock, type) {
    if (!textBlock) return null;
    var lines = textBlock.split('\n');
    for (var i = 0; i < lines.length; i++) {
      var line = lines[i].toUpperCase().trim();
      if (line.indexOf(namePart.toUpperCase()) !== -1) {
        var prices = line.match(/(\d{2,5})/g);
        if (prices && prices.length > 0) {
          // If size is requested, try to pick the correct number based on known column order
          // Fallback: return the most likely middle-range price for oils
          return parseInt(prices[Math.min(2, prices.length - 1)]); 
        }
      }
    }
    return null;
  ...
  var mrp = extractPrice(mrpText, 'MRP');
  var dlp = extractPrice(listText, 'DLP');
  
  if (!mrp && !dlp) return null;
  
  var reply = '';
  if (mrp) reply += '*MRP:* Rs.' + mrp + '\n';
  if (dlp) reply += '*DLP (List Price):* Rs.' + dlp;
  return reply.trim();
}
...
async function loadAllData() {
  ...
  var mrpText = normalizeSizes(rawMrp).slice(0, 15000);
  var listText = normalizeSizes(rawList).slice(0, 15000);
...
async function getAIReply(userMsg, data, prompt) {
  ...
  try {
    var res = await axios.post('https://integrate.api.nvidia.com/v1/chat/completions', { model: 'meta/llama-3.1-70b-instruct', messages: [{ role: 'system', content: prompt + '\n\nCONTEXT DATA:\n' + data }, { role: 'user', content: userMsg }], max_tokens: 600, temperature: 0.1 }, { headers: { 'Authorization': 'Bearer ' + key, 'Accept': 'application/json', 'Content-Type': 'application/json' }, timeout: 25000 });
    var raw = res.data?.choices?.[0]?.message?.content; return sanitizeReply(raw) || 'Kuch error aaya.';
...
module.exports = async function(req, res) {
  ...
  try {
    ...
    // ✅ RATE QUERY (Direct Lookup First)
    if (intent.type === 'ai_rate') {
      var directRate = findRateDirectly(text, dataResult.mrpText, dataResult.listText);
      if (directRate) {
        var prodName = text.replace(/rate|batao|kya hai|mrp|dlp|list price|price|kitna|of|aur|and|ka/gi, '').replace(/\d+\.?\d*l/gi, '').trim();
        await sendText(from, '*Product:* ' + prodName + '\n' + directRate);
      } else {
        var aiReply = await getAIReply(text, dataResult.excelData, sysPrompt);
        if (!aiReply || aiReply.indexOf('Error') !== -1 || aiReply.indexOf('missing') !== -1) {
          await sendText(from, 'Thoda wait karein ya product naam aur size clear likhein (e.g., Activ 4T 20W-40 900ml).');
        } else {
    ...
    // ✅ INVOICE SEARCH (Now shows similar matches)
    var matches = searchInvoices(text, dataResult.invoiceMap);
    if (matches.length === 1) {
      var m = matches[0]; var f = m.rows[0];
      var prods = m.rows.map(function(r) { return r['Product Name'] + '(' + r['Product Volume'] + 'L)'; }).join(' + ');
      var tG = m.rows.reduce(function(s, r) { return s + (parseFloat(r['Total Value incl VAT/GST']) || 0); }, 0);
      var wG = m.rows.reduce(function(s, r) { return s + (parseFloat(r['Total Value Without GST']) || 0); }, 0);
      var cg = m.rows.reduce(function(s, r) { return s + (parseFloat(r['CGST Value']) || 0); }, 0);
    ...
    var aiReply = await getAIReply(text, dataResult.excelData, sysPrompt);
    if (!aiReply || aiReply.indexOf('Error') !== -1 || aiReply.indexOf('missing') !== -1) {
      await sendText(from, 'Thoda wait karein ya clear likhein.');
    } else {
      await sendText(from, aiReply);
    }
    return res.status(200).json({ status: 'ok' });
