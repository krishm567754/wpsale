...
// ✅ Sanitize path to remove illegal chars for Firebase
...
async function getSystemPrompt() {
  ...
  var def = 'Tu Laxmi hai - Shri Laxmi Auto Store, Bikaner ki WhatsApp Assistant.\n\n' +
    ...
    '3. SIZE MATCHING: Product name ke saath size EXACT match karo. Examples: 900ML, 1L, 4.5L, 9L.\n' +
    '   Example: "ACTIV SCOOTER 10W-30 600ML" ka rate alag hai.\n' +
    '4. Format: *Product:* Name (Size)\n*MRP:* Rs.X\n*DLP:* Rs.Y\n5. Amounts ke saath Rs. lagao. Hinglish, max 5 lines.';
  if (!d) return def;
...
// ✅ NEW: Smart Name Matching (Keyword Intersection)
function searchInvoices(query, invoiceMap) {
  // Clean query: remove special chars, lowercase
  var q = query.replace(/[^a-zA-Z0-9\/\- ]/g, '').toLowerCase().trim();
  var matches = []; 
  var keys = Object.keys(invoiceMap);
  
  // Split query into significant words (length > 3)
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    var allMatch = userKeywords.every(function(word) {
      return custName.indexOf(word) !== -1;
    });
  ...
  // Sort: Exact matches first? (Length of match is harder to judge, just return top 5)
  return matches.slice(0, 5);
}

// ✅ NEW: Direct Rate Lookup (Bypasses AI for accuracy)
function findRateDirectly(query, mrpText, listText) {
  if (!mrpText || !listText) return null;
  
  // Clean query
  var q = query.toLowerCase().replace(/rate|batao|bhi|hai|kya|mrp|dlp|list|price|of|ka|inr|rs/gi, ' ').trim();
  var parts = q.split(/\s+/).filter(function(p){ return p.length > 1; });
  if (parts.length < 1) return null;

  // Identify Size (e.g., 900ml, 1l, 4.5l)
  var sizeMatch = q.match(/(\d+\.?\d*ml|\d+\.?\d*l)/i);
  var targetSize = sizeMatch ? sizeMatch[1].replace(/\s/g,'').toUpperCase() : null;
  
  // Identify Product Name (remove size from parts)
  var nameParts = parts.filter(function(p){ return !p.match(/^(ml|l|m)$/i); });
  var targetName = nameParts.join(' ').toUpperCase();

  if (!targetName) return null;

  // Helper to find price
  function findPriceInText(textBlock, isMRP) {
    if (!textBlock) return null;
    var lines = textBlock.split('\n');
    for (var i = 0; i < lines.length; i++) {
      var line = lines[i].toUpperCase().trim();
      // Check if line contains the Product Name
      if (line.indexOf(targetName) !== -1) {
        // If size is requested, check if size is in the line or header nearby?
        // PDF text is messy, usually size is not in the product row in these specific tables.
        // We rely on the column structure which is lost in raw text. 
        // BETTER STRATEGY: Use AI for this complex task, but ensure AI gets CLEAN text.
        // However, for simple exact matches:
        if (targetSize && line.indexOf(targetSize) === -1) continue; // Strict check
        
        // Extract numbers
        var prices = line.match(/(\d{2,6})/g);
  ...
  // Since column alignment is lost in PDF text extraction, Direct Lookup is risky for specific sizes
  // UNLESS the size is part of the product name (e.g., "GREASE AP3 50GM").
  // For OILS (1L, 900ML), the columns are headers. Direct lookup often fails or returns wrong col.
  
  // RETURNING NULL here forces the AI to handle it with the Context I will provide.
  // This prevents the "Wrong Price" issue caused by bad regex parsing.
  return null; 
}
...
async function loadAllData() {
  ...
  for (var k=0; k<excelFiles.length; k++) {
    try { 
      var wb = XLSX.read((await axios.get(base+'/'+encodeURIComponent(excelFiles[k]), {responseType:'arraybuffer'})).data, {type:'buffer'}); 
  ...
  // ✅ Clean Data Context for AI (Separate Sections clearly)
  var excelData = [lines.join('\n'), mrpText?'\n\n[MRP DATA SECTION - ONLY USE FOR MRP REQUESTS]\n'+mrpText:'', listText?'\n\n[DLP/LIST PRICE DATA SECTION - ONLY USE FOR DLP/LIST PRICE REQUESTS]\n'+listText:''].join('');
...
function detectIntent(text, mrpPdfUrl, listPdfUrl, mrpFile, listFile, savedPDFs) {
  ...
  // ✅ PDF Intent (Only if explicitly asked)
  ...
  // ✅ Rate Intent (If words like rate/price exist)
...
module.exports = async function(req, res) {
  ...
  try {
    ...
    // ✅ RATE QUERY LOGIC (Priority over Invoice Search)
    // If intent is rate, DO NOT search invoices.
    if (intent.type === 'ai_rate') {
      // Optional: Check direct match (disabled to avoid column errors, relying on AI with clean context)
    ...
    // ✅ INVOICE SEARCH LOGIC (Only for non-rate queries)
