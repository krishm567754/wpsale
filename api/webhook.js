...
async function sendDocument(to, fileUrl, fileName, caption = '') {
    ...
    try {
        const r = await fetch(`${baseUrl}/message/sendMedia/${instance}`, {
            method:  'POST',
            headers: { 'Content-Type': 'application/json', 'apikey': apiKey },
            body:    JSON.stringify({ number, mediatype: 'document', mimetype: 'application/pdf', media: fileUrl, fileName, caption }),
        });
        console.log('[PDF] Status:', r.status);
    } catch (e) { console.error('[PDF] Error:', e.message); }
}

// â”€â”€â”€ AI REPLY â€” GOOGLE GEMINI â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function getAIReply(userMessage, excelData, pdfList, systemPrompt) {
    const key = process.env.GOOGLE_AI_API_KEY;
    if (!key) { console.error('[AI] GOOGLE_AI_API_KEY missing!'); return 'API key missing.'; }

    const pdfCtx = Object.keys(pdfList).length
        ? `\nAvailable PDFs:\n${Object.entries(pdfList).map(([k,v]) => `- ${v.name} [keyword: ${k}]`).join('\n')}\n`
        : '';

    // Gemini model â€” gemini-2.0-flash free hai aur fast bhi
    const model = 'gemini-2.0-flash';
    const url   = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${key}`;

    const fullPrompt = `${systemPrompt}${pdfCtx}\n\nBUSINESS DATA (Excel):\n${excelData}`;
    ...
    try {
        ...
        const r = await fetch(url, {
            ...
            body: JSON.stringify({
                ...
                generationConfig: {
                    maxOutputTokens: 500,
                    temperature: 0.1,   // Low temperature = more accurate/factual
                },
            }),
        });

        console.log('[AI] Gemini status:', r.status);
        const d = await r.json();

        if (d.error) {
            console.error('[AI] Gemini error:', d.error.message);
