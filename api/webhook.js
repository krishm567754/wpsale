...
async function getSystemPrompt() {
    ...
    const defaultPrompt = 'Tu "Laxmi" hai - Shri Laxmi Auto Store, Bikaner (Castrol Distributor) ki WhatsApp Sales Assistant.\n\n' +
        ...
        '5. WhatsApp bold ke liye sirf *asterisk* use karo. Emojis, markdown tables, ya extra symbols mat use karo.\n' +
...
async function loadAllData() {
    ...
    if (mrpPdfUrl) {
        ...
        mrpText = raw.slice(0, 8000);
    }
    if (listPdfUrl) {
        const raw = await extractPDFText(listPdfUrl);
        listText = raw.slice(0, 8000);
    }
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // Handle numbered selection from previous list
        const database = getFirebase();
        if (database && /^\d+$/.test(text)) {
            ...
            if (pendingSnap.exists()) {
                ...
                if (pending.matches && pending.matches[idx]) {
                    ...
                    const products = match.rows.map(r => r['Product Name'] + '(' + r['Product Volume'] + 'L)').join(' + ');
                    const totalGST = match.rows.reduce((s, r) => s + (parseFloat(r['Total Value incl VAT/GST']) || 0), 0);
                    const woGST = match.rows.reduce((s, r) => s + (parseFloat(r['Total Value Without GST']) || 0), 0);
                    const cgst = match.rows.reduce((s, r) => s + (parseFloat(r['CGST Value']) || 0), 0);
                    const sgst = match.rows.reduce((s, r) => s + (parseFloat(r['SGST Value']) || 0), 0);
                    const vol = match.rows.reduce((s, r) => s + (parseFloat(r['Product Volume']) || 0), 0);
                    const date = cleanDate(first['Invoice Date']);
                    const reply = '*Invoice:* ' + match.invNo + '\n*Customer:* ' + first['Customer Name'] + '\n*Products:* ' + products + '\n*Total (with GST):* Rs.' + totalGST.toFixed(2) + '\n*Without GST:* Rs.' + woGST.toFixed(2) + '\n*Tax:* CGST Rs.' + cgst.toFixed(2) + ' + SGST Rs.' + sgst.toFixed(2) + '\n*Volume:* ' + vol.toFixed(1) + 'L\n*Date:* ' + date + '\n*Payment:* ' + first['Mode Of Payement'];
        ...
        if (matches.length === 1) {
            ...
            matches.forEach((m, i) => {
                msg += (i + 1) + '. ' + (m.customer || m.rows[0]['Customer Name']) + ' (Inv: ' + m.invNo + ')\n';
            });
            if (database) {
        ...
        // Fallback to AI for rates/general
