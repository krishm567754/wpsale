...
- Emojis mat use karo — sirf plain text + *bold*`;
...
async function getPDFList() {
    ...
    try {
        ...
        console.error('[PDF LIST] Error:', e.message);
        return {}; 
    }
}
...
async function loadAllData() {
    ...
    if (!base) {
        return { excelData: 'Data URL missing.', mrpPdfUrl: '', listPdfUrl: '', invoiceMap: {} };
    }
    ...
    for (const f of excelFiles) {
        try {
            ...
            console.log(`[EXCEL] Skip: ${f} - ${err.message}`); 
        }
    }
    ...
    // Extract PDF text (limited to 6000 chars)
    let mrpText = '';
    let listText = '';
    ...
    if (mrpPdfUrl) {
        try {
            ...
            console.error('[PDF] MRP extract error:', e.message);
        }
    }
    ...
    if (listPdfUrl) {
        try {
            ...
            console.error('[PDF] List extract error:', e.message);
        }
    }
...
async function getAIReply(userMsg, data, prompt) {
    ...
    if (!key) {
        return 'NVIDIA_API_KEY missing in Vercel variables.';
    }
    ...
    try {
        ...
        if (e.response) {
            return `AI Error ${e.response.status}: ${e.response.data?.error?.message || ''}`;
        }
