...
function cleanDate(val) {
  ...
  if (typeof val === 'number') {
    dt = new Date(Math.round((val - 25569) * 86400000));
  } else {
    dt = new Date(val);
  }
...
module.exports = async function(req, res) {
  ...
  try {
    ...
    const adminNum = process.env.ADMIN_NUMBER || '919950858818';
    ...
    // MEMORY LOGIC FOR PENDING OPTIONS (1, 2, 3...)
    if (/^\d+$/.test(text)) {
      let pending = null;
      if (database) {
        try {
    ...
    for (const k in savedPDFs) { if (lower.includes(k.toLowerCase()) && hasSend) { await sendText(from, `Sending ${savedPDFs[k].name}...`); await sendDocument(from, savedPDFs[k].url, savedPDFs[k].name, savedPDFs[k].name); return res.status(200).json({ status: 'ok' }); } }
