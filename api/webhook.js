...
function normalizeSizes(text) {
  if (!text) return '';
  return text.replace(/900\s*ml/gi,'900ML').replace(/800\s*ml/gi,'800ML').replace(/600\s*ml/gi,'600ML').replace(/500\s*ml/gi,'500ML').replace(/1\s*l\b/gi,'1L').replace(/4\s*\.?\s*5l\b/gi,'4.5L').replace(/9\s*l\b/gi,'9L');
}
...
function searchInvoices(query, invoiceMap) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (invClean.indexOf(qClean) !== -1 || qClean.indexOf(invClean) !== -1 || cust.indexOf(q) !== -1 || q.split(' ').some(function(w){return w.length>2 && cust.indexOf(w)!==-1;})) {
...
function findRateDirectly(query, mrpText, listText) {
  ...
  var q = query.toLowerCase().replace(/mrp|dlp|list|price|rate|kitna|kya hai|ka|of/gi, ' ').trim();
  var parts = q.split(/\s+/).filter(function(p){return p.length>2;});
  if (parts.length < 1) return null;
  var prod = parts.join(' ');
  var sizeMatch = prod.match(/(\d+\.?\d*ml|1l|4\.5l|9l)/i);
  var size = sizeMatch ? sizeMatch[1].toUpperCase().replace(/\s/g,'') : '';
  if (!size) return null;
  var lines = (mrpText + '\n' + listText).split('\n');
  for (var i=0; i<lines.length; i++) {
    var line = lines[i].toUpperCase().trim();
    if (line.indexOf(prod.toUpperCase()) !== -1 && line.indexOf(size) !== -1) {
      var mrpLine = mrpText.split('\n').find(function(l){ return l.toUpperCase().indexOf(prod.toUpperCase()) !== -1 && l.toUpperCase().indexOf(size) !== -1; });
      var dlpLine = listText.split('\n').find(function(l){ return l.toUpperCase().indexOf(prod.toUpperCase()) !== -1 && l.toUpperCase().indexOf(size) !== -1; });
      var mrp = mrpLine ? (mrpLine.match(/(\d{2,5})/g) || []).pop() : null;
      var dlp = dlpLine ? (dlpLine.match(/(\d{2,5})/g) || []).pop() : null;
      var rep = '*Product:* ' + prod.toUpperCase() + ' (' + size + ')\n';
      if (mrp) rep += '*MRP:* Rs.' + mrp + '\n';
      if (dlp) rep += '*DLP:* Rs.' + dlp;
      return rep.trim();
    }
  }
  return null;
...
async function extractPDFText(url) {
  try { var res = await axios.get(url, { responseType: 'arraybuffer', timeout: 20000 }); var pdfParse = require('pdf-parse'); var data = await pdfParse(Buffer.from(res.data)); return data.text || ''; } catch (e) { console.error('[PDF] Err:', e.message); return ''; }
}

async function loadAllData() {
  var base = process.env.GITHUB_RAW_BASE;
  if (!base) return { excelData: 'Missing', mrpPdfUrl: '', listPdfUrl: '', invoiceMap: {}, mrpText: '', listText: '' };
  var fileList = []; try { fileList = (await axios.get(base+'/index.json')).data; } catch(e) { return { excelData: 'Error', mrpPdfUrl: '', listPdfUrl: '', invoiceMap: {}, mrpText: '', listText: '' }; }
  ...
  for (var k=0; k<excelFiles.length; k++) {
    try { var wb = XLSX.read((await axios.get(base+'/'+encodeURIComponent(excelFiles[k]), {responseType:'arraybuffer'})).data, {type:'buffer'}); for (var s=0; s<wb.SheetNames.length; s++) { allRows = allRows.concat(XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[s]], {defval:''})); } } catch(e){}
  }
  ...
  var invKeys = Object.keys(invoiceMap); for (var n=0; n<invKeys.length; n++) { var invNo=invKeys[n]; var rows=invoiceMap[invNo]; var f=rows[0]; var prods=rows.map(function(r){return r['Product Name']+'('+r['Product Volume']+'L)';}).join(' + '); var tG=rows.reduce(function(s,r){return s+(parseFloat(r['Total Value incl VAT/GST'])||0);},0); var wG=rows.reduce(function(s,r){return s+(parseFloat(r['Total Value Without GST'])||0);},0); var cg=rows.reduce(function(s,r){return s+(parseFloat(r['CGST Value'])||0);},0); var sg=rows.reduce(function(s,r){return s+(parseFloat(r['SGST Value'])||0);},0); var vl=rows.reduce(function(s,r){return s+(parseFloat(r['Product Volume'])||0);},0); lines.push(invNo+'|'+cleanDate(f['Invoice Date'])+'|'+f['Customer Name']+'|'+f['Town Name']+'|'+f['District Name']+'|'+f['Sales Executive Name']+'|'+prods+'|'+vl.toFixed(1)+'L|Rs.'+tG.toFixed(2)+'|Rs.'+wG.toFixed(2)+'|Rs.'+cg.toFixed(2)+'|Rs.'+sg.toFixed(2)+'|'+f['Mode Of Payement']); }
  var mrpText = mrpPdfUrl ? normalizeSizes((await extractPDFText(mrpPdfUrl))).slice(0,15000) : '';
  var listText = listPdfUrl ? normalizeSizes((await extractPDFText(listPdfUrl))).slice(0,15000) : '';
...
async function getAIReply(userMsg, data, prompt) {
  ...
  try { var res = await axios.post('https://integrate.api.nvidia.com/v1/chat/completions', { model:'meta/llama-3.1-70b-instruct', messages:[{role:'system',content:prompt+'\n\nCONTEXT DATA:\n'+data},{role:'user',content:userMsg}], max_tokens:600, temperature:0.1 }, { headers:{'Authorization':'Bearer '+key,'Accept':'application/json','Content-Type':'application/json'}, timeout:25000 }); var raw = res.data?.choices?.[0]?.message?.content; return sanitizeReply(raw) || 'Kuch error aaya.'; } catch(e) { return 'System Error: '+e.message; }
}
...
async function sendText(to, text) {
  ...
  if(!baseUrl||!inst||!key) return; try { await axios.post(baseUrl+'/message/sendText/'+inst, {number:num, text:text}, {headers:{'Content-Type':'application/json','apikey':key}}); } catch(e){ console.error('[SEND] Err:', e.message); }
}

async function sendDocument(to, fileUrl, fileName, caption) {
  var baseUrl = (process.env.EVOLUTION_API_URL||'').replace(/\/$/,''); var inst = process.env.EVOLUTION_INSTANCE; var key = process.env.EVOLUTION_API_KEY; var num = to.replace(/@s\.whatsapp\.net$/,'').replace(/@g\.us$/,'');
  if(!baseUrl||!inst||!key) return; try { await axios.post(baseUrl+'/message/sendMedia/'+inst, {number:num, mediatype:'document', mimetype:'application/pdf', media:fileUrl, fileName:fileName, caption:caption||''}, {headers:{'Content-Type':'application/json','apikey':key}}); } catch(e){ console.error('[PDF] Err:', e.message); }
}
...
module.exports = async function(req, res) {
  ...
  try {
    ...
    var from = body.data.key.remoteJid; var text = ((body.data.message && body.data.message.conversation) || (body.data.message && body.data.message.extendedTextMessage && body.data.message.extendedTextMessage.text) || '').trim();
    if (!text || !from) return res.status(200).send('Empty');
    var adminNum = process.env.ADMIN_NUMBER || '916375636354'; var isAdmin = from.indexOf(adminNum) !== -1; var safeFrom = sanitizePath(from);
    ...
    var lower = text.toLowerCase(); if(['hi','hello','namaste','hey','hii','good morning'].some(function(g){return lower.indexOf(g)!==-1;})) { await sendText(from,'Hello! Main Laxmi hoon, Shri Laxmi Auto Store ki assistant. Invoice details, MRP/DLP rates, ya koi bhi query pooch sakte hain!'); return res.status(200).json({status:'ok'}); }

    var database = getFirebase(); if(database && /^\d+$/.test(text)) { try { var snap = await database.ref('pending/'+safeFrom).get(); if(snap.exists()) { var pending=snap.val(); var idx=parseInt(text)-1; if(pending.matches && pending.matches[idx]) { var m=pending.matches[idx]; var f=m.rows[0]; var prods=m.rows.map(function(r){return r['Product Name']+'('+r['Product Volume']+'L)';}).join(' + '); var tG=m.rows.reduce(function(s,r){return s+(parseFloat(r['Total Value incl VAT/GST'])||0);},0); var wG=m.rows.reduce(function(s,r){return s+(parseFloat(r['Total Value Without GST'])||0);},0); var cg=m.rows.reduce(function(s,r){return s+(parseFloat(r['CGST Value'])||0);},0); var sg=m.rows.reduce(function(s,r){return s+(parseFloat(r['SGST Value'])||0);},0); var vl=m.rows.reduce(function(s,r){return s+(parseFloat(r['Product Volume'])||0);},0); await sendText(from,'*Invoice:* '+m.invNo+'\n*Customer:* '+f['Customer Name']+'\n*Products:* '+prods+'\n*Total:* Rs.'+tG.toFixed(2)+'\n*Tax:* CGST Rs.'+cg.toFixed(2)+' + SGST Rs.'+sg.toFixed(2)+'\n*Date:* '+cleanDate(f['Invoice Date'])+'\n*Payment:* '+f['Mode Of Payement']); await database.ref('pending/'+safeFrom).remove(); return res.status(200).json({status:'ok'}); } } } catch(e){} }
    ...
    if(matches.length === 1) { var m=matches[0]; var f=m.rows[0]; var prods=m.rows.map(function(r){return r['Product Name']+'('+r['Product Volume']+'L)';}).join(' + '); var tG=m.rows.reduce(function(s,r){return s+(parseFloat(r['Total Value incl VAT/GST'])||0);},0); var wG=m.rows.reduce(function(s,r){return s+(parseFloat(r['Total Value Without GST'])||0);},0); var cg=m.rows.reduce(function(s,r){return s+(parseFloat(r['CGST Value'])||0);},0); var sg=m.rows.reduce(function(s,r){return s+(parseFloat(r['SGST Value'])||0);},0); var vl=m.rows.reduce(function(s,r){return s+(parseFloat(r['Product Volume'])||0);},0); await sendText(from,'*Invoice:* '+m.invNo+'\n*Customer:* '+f['Customer Name']+'\n*Products:* '+prods+'\n*Total:* Rs.'+tG.toFixed(2)+'\n*Tax:* CGST Rs.'+cg.toFixed(2)+' + SGST Rs.'+sg.toFixed(2)+'\n*Date:* '+cleanDate(f['Invoice Date'])+'\n*Payment:* '+f['Mode Of Payement']); return res.status(200).json({status:'ok'}); }
    else if(matches.length > 1) { var msg='*Multiple matches. Reply 1, 2, or 3:\n\n'; for(var i=0;i<matches.length;i++){ msg+=(i+1)+'. '+matches[i].customer+' (Inv: '+matches[i].invNo+')\n'; } if(database){try{await database.ref('pending/'+safeFrom).set({matches:matches,ts:Date.now()});}catch(e){}} await sendText(from,msg); return res.status(200).json({status:'ok'}); }
    else if(matches.length === 0 && (text.match(/^\d+$/) || text.toLowerCase().indexOf('inv') !== -1)) { await sendText(from,'Invoice nahi mila. Number ya customer name check karke dobara try karein.'); return res.status(200).json({status:'ok'}); }

    var directRate = findRateDirectly(text, dataResult.mrpText, dataResult.listText);
    if(directRate) { var prodName = text.replace(/mrp|dlp|list price|rate|kya hai|kitna|price|ka|of/gi,'').trim(); await sendText(from,'*Product:* '+prodName.toUpperCase()+'\n'+directRate); return res.status(200).json({status:'ok'}); }
