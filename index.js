...
// Load Excel File from GitHub Root
...
async function startBot() {
  console.log('🚀 Starting Castrol Sales Bot with Gemini 2.0...');
  
  try { await initFirebase(); } catch(e) { console.log("Firebase Init Skip/Warning"); }
    ...
    for (const msg of messages) {
      ...
      try {
        ...
        if (['hi','hello','hey'].includes(text.toLowerCase())) {
          await sock.sendMessage(sender, { text: "Hello! 👋 Main Shri Laxmi Auto Store ka sales bot hoon. Poochiye kis party ka data chahiye?" });
          continue;
        }
        ...
        let instructions = "You are a sales assistant for a Castrol Distributor. Answer in Hinglish.";
        try {
          instructions = await Promise.race([
            getInstructions(),
            new Promise((_, r) => setTimeout(() => r(), 2000))
          ]);
        } catch (e) { console.log("Using Default Instructions due to delay."); }
