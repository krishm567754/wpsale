  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      try {
        ...
        // ── 1. GREETING HANDLER (HI/HELLO) ──────────────────────────────────
        const greetings = ['hi', 'hello', 'hey', 'namaste', 'hlo', 'hii'];
        if (greetings.includes(text.toLowerCase())) {
            const welcomeMsg = `Hello ${userName}! 👋\n\nMain Shri Laxmi Auto Store ka sales assistant hoon. Aap mujhse invoices, monthly sales ya Castrol schemes ke baare mein pooch sakte hain.\n\nMain aapki kya madad kar sakta hoon?`;
